...
func (*BasicFunctionalitySuite) Scenario_CheckVersion(svm *ScenarioVariationManager) {
	...
	if !svm.Dryrun() {
		...
		if !foundVersionInOutput() {
			svm.Log("Expected to find version info in stdout, but did not. stdout: %v", stdout.RawStdout())
...
func (*BasicFunctionalitySuite) Scenario_DisabledCheckVersion(svm *ScenarioVariationManager) {
	...
	if !svm.Dryrun() {
		...
		versionAssertion.False(foundVersionInOutput(), "Expected to NOT find version info in stdout, but did. stdout: %v", stdout.RawStdout())
	}
