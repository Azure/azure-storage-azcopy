...
func (s *FilesNFSTestSuite) Scenario_AzureSMBToAzureNFS(svm *ScenarioVariationManager) {
	...
	// To print the actual values instead of pointer addresses:
	if fileProperties.FileCreationTime != nil {
		fmt.Println("File Creation Time Value:", fileProperties.FileCreationTime.Format(time.RFC3339))
	}
	if fileProperties.FileLastWriteTime != nil {
		fmt.Println("File LastWrite Time Value:", fileProperties.FileLastWriteTime.Format(time.RFC3339))
	}
	if folderProperties.FileCreationTime != nil {
		fmt.Println("Folder Creation Time Value:", folderProperties.FileCreationTime.Format(time.RFC3339))
	}
	rootDir := "dir_file_copy_test_" + uuid.NewString()
	...
	if preserveProperties {
		srcProp := srcObjs[rootDir+"/test1.txt"].ObjectProperties
		fmt.Print("Source File Creation Time:", srcProp.FileProperties.FileCreationTime, "\n")
		fmt.Print("Source File LastWrite Time:", srcProp.FileProperties.FileLastWriteTime, "\n")

		//dstProp := dstShare.GetObject(svm, rootDir+"/test1.txt", common.EEntityType.File()).GetProperties(svm)
		//fmt.Print("Destination File Creation Time:", dstProp.FileNFSProperties.FileCreationTime, "\n")
		//fmt.Print("Destination File LastWrite Time:", dstProp.FileNFSProperties.FileLastWriteTime, "\n")
	}
