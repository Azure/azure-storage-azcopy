...
func TestTelemetryTransportFailuresStopPipeline(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			agent := &telemetryAgent{enabled: true, reporter: telemetry.NewReporter(telemetry.Config{ConnectionString: "InstrumentationKey=test;IngestionEndpoint=https://example.test", HTTPClient: client})}
...
func TestTelemetryDeliveryFailureCancelsActiveRequests(t *testing.T) {
	...
	agent := &telemetryAgent{enabled: true, sendTimeout: time.Minute, reporter: telemetry.NewReporter(telemetry.Config{ConnectionString: "InstrumentationKey=test;IngestionEndpoint=" + server.URL, HTTPClient: server.Client()})}
