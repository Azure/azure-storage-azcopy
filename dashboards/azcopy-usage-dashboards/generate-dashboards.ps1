...
let FilteredFinishedJobs = materialize(
    ...
    | where $__contains(SourceEndpointKind, ${SourceEndpointKind})
...
function New-QueryVariable([string]$Name, [string]$Label, [string]$Query) {
    return [ordered]@{
        name = $Name
        label = $Label
        description = if ($Name -in @('SourceType', 'SourceMountType', 'SourceEndpointKind', 'DestType', 'DestEndpointKind', 'InvocationContext')) { Get-AzCopyFieldDescription @($Name) } else { '' }
        type = "query"
...
function New-AdxDashboard([hashtable]$Definition) {
    ...
    foreach ($variable in $queryVariables) {
        ...
        $parameters += [ordered]@{
            ...
            description = if ($variable.Name -in @('SourceType', 'DestType', 'InvocationContext')) { Get-AzCopyFieldDescription @($variable.Name) } else { '' }
...
let Events = materialize(
    ...
    | where tostring(customDimensions.SchemaVersion) in ('1', '2', '3')
...
$dataManifest = @(
    ...
    @{ Title = "Data by source endpoint kind"; Path = "queries\tiles\19_data_by_source_endpoint.kql"; Visual = "bar"; Description = "Logical payload by source hostname classification."; X = 0; Y = 91; W = 22; H = 8 }
)

foreach ($item in $dataManifest) {
    $fields = switch ($item.Path) {
        "queries\tiles\05_data_by_source_target.kql" { @('SourceType', 'DestType') }
        "queries\tiles\06_objects_by_source_target.kql" { @('SourceType', 'DestType') }
        "queries\tiles\07_data_by_destination_type.kql" { @('DestType') }
        "queries\tiles\09_data_by_destination_endpoint.kql" { @('DestEndpointKind') }
        "queries\tiles\08_data_by_source_mount.kql" { @('SourceMountType') }
        "queries\tiles\11_recent_finished_jobs.kql" { @('SourceType', 'SourceEndpointKind', 'DestType', 'DestEndpointKind') }
        "queries\tiles\19_data_by_source_endpoint.kql" { @('SourceEndpointKind') }
    }
    if ($fields) {
...
function New-DataGrafanaDashboard {
    ...
    $parameterMap = @(
        ...
        @{ Name = "SourceEndpointKind"; Label = "Source endpoint kind"; Path = "queries\parameters\07_source_endpoint_kind.kql" },
