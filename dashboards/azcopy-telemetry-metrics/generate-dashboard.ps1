...
**azcopy.source_bytes_scanned** sums listed sizes of eligible source payload objects after traversal filters, before sync decides which need transfer. It includes unchanged sync objects, not filtered-out objects, destination scanning, or folder-property operations. It is not payload bytes read or total account size. **BytesScanned** and **Source bytes scanned** name this same counter.

Example: 100 GiB of source files, 20 GiB excluded by filters, and 60 GiB already up to date gives 80 GiB source bytes scanned and 20 GiB bytes enumerated (scheduled), assuming ordinary files and a complete scan. Resume summary counters can be job-cumulative.
'@
...
- **Job error distribution**: Count of finished job attempts grouped by terminal error category/code, terminal reason, and stage. This is job-level failure context, not a count of individual failed files or HTTP attempts.

Counts are received telemetry observations. Percentage lines are averages across jobs, not ratios recomputed from the hourly count totals.
'@
...
$manifest = @(
    ...
    @{ Page = "Performance"; Title = "Configured concurrency"; Description = "Distribution of AZCOPY_CONCURRENCY_VALUE recorded in OptConcurrency. Runs without an explicit setting appear as 'not set'."; File = "queries/34_concurrency_distribution.kql"; Visual = "column"; XColumn = "Value"; YColumns = @("Attempts"); X = 0; Y = 27; W = 22; H = 9 },
    @{ Page = "Performance"; Title = "Configured buffer size (GB)"; Description = "Distribution of AZCOPY_BUFFER_GB recorded in OptBufferGB. Runs without an explicit setting appear as 'not set'."; File = "queries/35_buffer_gb_distribution.kql"; Visual = "column"; XColumn = "Value"; YColumns = @("Attempts"); X = 0; Y = 36; W = 11; H = 9 },
    @{ Page = "Performance"; Title = "Configured throughput cap (Mbps)"; Description = "Distribution of --cap-mbps recorded in OptCapMbps. Runs without an explicit setting appear as 'not set'."; File = "queries/36_cap_mbps_distribution.kql"; Visual = "column"; XColumn = "Value"; YColumns = @("Attempts"); X = 11; Y = 36; W = 11; H = 9 },
...
foreach ($item in $manifest) {
    $fields = switch ($item.Title) {
        "Transfer dimension mix" { @('SourceType', 'DestType') }
        ...
        "Storage latency and IOPS" { @('azcopy.storage_http_attempt_count', 'azcopy.avg_iops') }
        "HTTP, network, and overflow error counts" { @('azcopy.storage_http_attempt_count', 'azcopy.avg_iops') }
        "Invocation context" { @('InvocationContext') }
        "Combined host and runtime details" { @('InvocationContext') }
    }
    if ($fields) {
        $item.Description = (@($item.Description, (Get-AzCopyFieldDescription $fields)) | Where-Object { $_ }) -join "`n`n"
    }
...
Write-Host "Generated $OutputPath with $($dashboard.tiles.Count) tiles across $($pages.Count) pages covering $($expectedMetrics.Count) emitted metrics."