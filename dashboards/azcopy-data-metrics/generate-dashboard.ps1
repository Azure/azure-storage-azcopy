...
"@
    ...
    @{
        Title = "Data by source endpoint kind"
        File = "queries/tiles/19_data_by_source_endpoint.kql"
        Visual = "bar"
        Description = "Logical payload volume grouped by source hostname classification."
        X = 0; Y = 91; W = 22; H = 8
    }
)
...
foreach ($item in $manifest) {
    ...
    $fields = switch ($item.File) {
        "queries/tiles/05_data_by_source_target.kql" { @('SourceType', 'DestType') }
        "queries/tiles/06_objects_by_source_target.kql" { @('SourceType', 'DestType') }
        "queries/tiles/07_data_by_destination_type.kql" { @('DestType') }
        "queries/tiles/09_data_by_destination_endpoint.kql" { @('DestEndpointKind') }
        "queries/tiles/08_data_by_source_mount.kql" { @('SourceMountType') }
        "queries/tiles/11_recent_finished_jobs.kql" { @('SourceType', 'SourceEndpointKind', 'DestType', 'DestEndpointKind') }
        "queries/tiles/19_data_by_source_endpoint.kql" { @('SourceEndpointKind') }
    }
    if ($fields) {
        $item.Description += "`n`n" + (Get-AzCopyFieldDescription $fields)
    }
...
$parameterDefinitions = @(
    @{ VariableName = "_sourceType"; DisplayName = "Source type"; File = "queries/parameters/01_source_type.kql"; Description = (Get-AzCopyFieldDescription @('SourceType')) },
    @{ VariableName = "_destType"; DisplayName = "Destination type"; File = "queries/parameters/02_destination_type.kql"; Description = (Get-AzCopyFieldDescription @('DestType')) },
    @{ VariableName = "_fromTo"; DisplayName = "From-to"; File = "queries/parameters/03_from_to.kql"; Description = "Exact AzCopy FromTo pairing." },
    @{ VariableName = "_sourceMountType"; DisplayName = "Source mount type"; File = "queries/parameters/04_source_mount_type.kql"; Description = (Get-AzCopyFieldDescription @('SourceMountType')) },
    @{ VariableName = "_sourceEndpointKind"; DisplayName = "Source endpoint kind"; File = "queries/parameters/07_source_endpoint_kind.kql"; Description = (Get-AzCopyFieldDescription @('SourceEndpointKind')) },
    @{ VariableName = "_destEndpointKind"; DisplayName = "Destination endpoint kind"; File = "queries/parameters/05_destination_endpoint_kind.kql"; Description = (Get-AzCopyFieldDescription @('DestEndpointKind')) },
...
Write-Host "Generated $OutputPath with $($dashboard.tiles.Count) tiles, $($parameters.Count) parameters, and $($queries.Count) queries."
