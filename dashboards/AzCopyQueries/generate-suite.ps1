...
foreach ($definition in $definitions) {
    ...
    foreach ($entry in $variableNames.GetEnumerator()) {
        ...
        $grafanaVariables.Add(@{
            ...
            description = if ($entry.Value -in @('SourceType', 'TargetType')) { Get-AzCopyFieldDescription @($entry.Value) } else { '' }
        ...
        $adxParameters.Add(@{
            id = (New-StableId $dashboardIndex (10 + $parameterIndex)); kind = 'string'; displayName = $entry.Value
            description = if ($entry.Value -in @('SourceType', 'TargetType')) { Get-AzCopyFieldDescription @($entry.Value) } else { '' }
    ...
    foreach ($panel in $definition.Panels) {
        ...
        if ($panel.Query -match "AzCopy_(UsageDetails|NoSuccessDetails)\(|\bTargets\b|'(target|topology)'\)") {
            $grafanaPanel.description += "`n`n" + (Get-AzCopyFieldDescription @('TargetType'))
        }
        if ($panel.Query -match "AzCopy_(UsageDetails|NoSuccessDetails)\(|\bSources\b|'(source|topology)'\)") {
            $grafanaPanel.description += "`n`n" + (Get-AzCopyFieldDescription @('SourceType'))
        }
    ...
    foreach ($format in @('grafana', 'adx')) {
        ...
        Add-AzCopyVisibleFieldGuides $artifact
    ...
    $manifest.Add(@{ uid = $definition.Uid; title = $definition.Title; sourceUid = $definition.SourceUid; sourceVersion = $definition.SourceVersion; grafanaPanels = $grafana.panels.Count; adxTiles = $adx.tiles.Count; queryFunctions = @($definition.Panels.Query | ForEach-Object { if ($_ -match '^(AzCopy_\w+)\(') { $Matches[1] } } | Sort-Object -Unique) })
    Write-Host "Generated $($definition.Title): $($grafana.panels.Count) Grafana panels, $($adx.tiles.Count) ADX tiles"
}
