...
foreach ($filterName in $filterVariables.Keys) {
    ...
    $parameters.Add([ordered]@{
        kind = 'string'; id = (Get-ArtifactId (10 + $filterIndex)); displayName = $filterName
        description = if ($filterName -in @('SourceType', 'TargetType')) { Get-AzCopyFieldDescription @($filterName) } else { '' }
