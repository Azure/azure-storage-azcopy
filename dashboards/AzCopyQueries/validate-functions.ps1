...
if ($dashboard.panels.Count -ne 9) { throw 'Dashboard must contain the two source rows, six bar gauges, and one details table.' }
...
if ($adxDashboard.tiles.Count -ne 7 -or $adxDashboard.queries.Count -ne 11 -or $adxDashboard.parameters.Count -ne 6) {
    throw 'ADX dashboard must contain seven tiles, eleven queries, and six parameters.'
}
