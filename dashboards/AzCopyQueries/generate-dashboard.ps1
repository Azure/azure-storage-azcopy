...
function New-DetailsPanel([string]$Query) {
    return [ordered]@{
        ...
        description = 'Detailed equivalent of the Storage Mover Top 100 panel for the selected datetime range.'
        type = 'table'
        datasource = [ordered]@{ type = 'grafana-azure-data-explorer-datasource'; uid = $DatasourceUid }
...
if ($parsed.uid -ne 'azcopy-top-movers' -or $parsed.panels.Count -ne 9 -or $parsed.templating.list.Count -ne 5) { throw 'Generated dashboard failed structural validation.' }
