...
$manifest = @(
    ...
    @{ Page = "Data Quality"; Title = "Acceptance definition"; MarkdownText = "**Accepted** is a client-telemetry integrity result only. It requires one supported schema (v1, v2, or v3), exactly one start and one finish, required fields, a complete finish metric set, a finish after its start, and a reported duration within the allowed tolerance (the greater of 60 seconds or 20%). Storage request evidence is not considered. Version 1 alone does not establish payload completeness."; Visual = "markdownCard"; X = 0; Y = 0; W = 22; H = 3 },
    @{ Page = "Data Quality"; Title = "Telemetry acceptance"; Description = "Client-telemetry integrity only; Storage request evidence is not considered. Accepted means the attempt uses one supported schema (v1, v2, or v3), has exactly one start and one finish, includes required fields and a complete finish metric set, finishes after it starts, and reports a duration within the allowed tolerance (the greater of 60 seconds or 20%)."; File = "queries/client/08_telemetry_quality.kql"; Visual = "multistat"; X = 0; Y = 3; W = 22; H = 6 },
    ...
    @{ Page = "Server Correlation"; Title = "Aggregate-correlation definition"; MarkdownText = "**Requests per job** joins client finished jobs and server-observed AzCopy requests by destination account and hour. Storage requests do not contain AzCopy JobID, so this is aggregate supporting context, not exact job attribution. The full outer join intentionally shows hours present on only one side."; Visual = "markdownCard"; X = 0; Y = 12; W = 22; H = 4 },
    ...
    @{ Page = "Server Correlation"; Title = "Requests per job"; File = "queries/server/04_client_server_account_hour.kql"; Visual = "table"; X = 10; Y = 16; W = 12; H = 10 }
)

foreach ($item in $manifest) {
    $minimumWidth = if ($item.Visual -eq "markdownCard") { 2 } else { 3 }
    $fields = switch ($item.Title) {
        "Platform and version mix" { @('SourceType', 'DestType') }
        "Source and destination platform mix" { @('SourceType', 'SourceProtocol', 'DestType', 'DestProtocol') }
        "Reliability rates" { @('azcopy.storage_http_attempt_count') }
        "Per-attempt Storage request evidence" { @('azcopy.storage_http_attempt_count') }
    }
    if ($fields) {
...
Write-Host "Generated $OutputPath with $($dashboard.tiles.Count) tiles across $($pages.Count) pages."