...
$manifest = @(
    ...
    @{ Page = "Adoption"; Title = "Observed sampled-installation behavior (proxy)"; Description = "Unique InstallationIDs with sampled finished jobs in the selected range. Rates show whether the first observed attempt succeeded, whether 2+ attempts were observed, and whether any observed attempt succeeded. Time cards measure first observed attempt to first observed success. This is not a true acquisition funnel or first-ever/second-ever job analysis because unsampled and out-of-range jobs are invisible."; File = "queries/client/15_observed_installation_funnel.kql"; Visual = "multistat"; X = 0; Y = 8; W = 22; H = 7 },
