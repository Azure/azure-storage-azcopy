...
def init():
    ...
    if check_env_not_exist('TEST_DIRECTORY_PATH') or check_env_not_exist('AZCOPY_EXECUTABLE_PATH') or \
            ...
            check_env_not_exist('AZCOPY_OAUTH_TOKEN_INFO') or check_env_not_exist('S2S_DST_FILE_ACCOUNT_SAS_URL') or \
    ...
    s2s_dst_file_account_url = os.environ.get('S2S_DST_FILE_ACCOUNT_SAS_URL')
    ...
    if not util.initialize_test_suite(test_dir_path, container_sas, container_oauth, container_oauth_validate, share_sas_url, premium_container_sas,
                                      filesystem_url, filesystem_sas_url, s2s_src_blob_account_url, s2s_src_file_account_url, s2s_src_s3_service_url, s2s_dst_blob_account_url, s2s_dst_file_account_url, azcopy_exec_location, test_suite_exec_location):
