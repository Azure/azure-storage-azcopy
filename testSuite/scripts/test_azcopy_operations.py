...
def test_cancel_job():
    ...
    output = util.Command("copy").add_arguments(file_path).add_flags("Logging", "info").add_flags("recursive", "true").add_flags("background-op", "true").execute_azcopy_copy_command_get_output()
    if output is None:
...
def test_remove_files_with_Wildcard():
    ...
    if not result:
        print("test_remove_files_with_Wildcard failed uploading directory dir_remove_files_with_wildcard to the container")
        return
