...
func createBucket(bucketURLStr string) {
	...
	if err := s3Client.MakeBucket(context.TODO(), s3URLParts.BucketName, minio.MakeBucketOptions{Region: s3URLParts.Region}); err != nil {
		exists, err := s3Client.BucketExists(context.TODO(), s3URLParts.BucketName)
		if err != nil || !exists {
...
func createObject(objectURLStr string, objectSize uint32, o minio.PutObjectOptions) {
	...
	_, err = s3Client.PutObject(context.TODO(), s3URLParts.BucketName, s3URLParts.ObjectKey, bytes.NewReader([]byte(randomString)), int64(objectSize), o)
