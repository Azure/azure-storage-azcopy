...
func main() {
	...
	highChunk := make(chan chunkMsg, 500)
	highTransfer := make(chan transferMsg, 500)
	...
	highTransfer <- transferMsg{
		...
		Destination:     "http://azcopynextgendev2.blob.core.windows.net/testcontainer/aaa10.pdf?st=2017-12-07T00%3A27%3A00Z&se=2018-12-08T00%3A27%3A00Z&sp=rwdl&sv=2016-05-31&sr=c&sig=D9xT4VAKVAHQYosYzKDY%2FaMhBrTIvlcxLORsPst6%2BuM%3D",
		...
		default:
			...
			default:
				...
				case transferMsg := <-highPriorityTransferChannel:
					fmt.Println("Worker", workerId, "is processing TRANSFER job with transferId", transferMsg.id)
					computePrologueFunc(transferMsg.SourceType, transferMsg.DestinationType)(transferMsg, highPriorityChunkChannel)
				default:
