...
type LifecycleMgr interface {
	...
	OnScanProgress(progress common.ScanProgress) // only called during sync jobs
	OnTransferProgress(progress common.TransferProgress)
	OnComplete(summary common.JobSummary)
	...
	Prompt(message string, details common.PromptDetails) common.ResponseOption
	Info(string)
	Warn(string)
	...
	Exit(common.OutputBuilder, common.ExitCode)            // indicates successful execution exit after printing, allow user to specify exit code
	Dryrun(common.OutputBuilder)                           // print files for dry run mode
	Output(common.OutputBuilder, common.OutputMessageType) // print output for list
	//WaitForCompletion() (JobSummary, error)  // wait for the job to complete, return the summary and error if any
	SurrenderControl()
	...
	SetOutputFormat(common.OutputFormat)      // change the output format of the entire application
	...
	SetOutputVerbosity(mode common.OutputVerbosity)
}
...
// glcm is the lifecycle manager for the AzCopy client. This needs to be reset per copy/sync/resume job for AzCopy as a library users.
...
var glcm LifecycleMgr = func() (lcmgr *lifecycleMgr) {
	...
	common.SetOutputHandler(&common.JobOutputHandler{
		Prompt:                 lcmgr.Prompt,
		Info:                   lcmgr.Info,
		Warn:                   lcmgr.Warn,
...
type lifecycleMgr struct {
	msgQueue              chan common.OutputMessage
...
func (lcm *lifecycleMgr) SetOutputFormat(format common.OutputFormat) {
...
func (lcm *lifecycleMgr) OnStart(ctx common.JobContext) {
	o := common.GetStandardInitOutputBuilder(ctx)
	lcm.msgQueue <- common.OutputMessage{
		MsgContent: o(lcm.outputFormat),
		MsgType:    common.EOutputMessageType.Init(),
	}
}

func (lcm *lifecycleMgr) OnScanProgress(progress common.ScanProgress) {
	o := common.GetScanProgressOutputBuilder(progress)
	lcm.msgQueue <- common.OutputMessage{
		MsgContent: o(lcm.outputFormat),
		MsgType:    common.EOutputMessageType.Progress(),
	}
}

func (lcm *lifecycleMgr) OnTransferProgress(progress common.TransferProgress) {
...
func (lcm *lifecycleMgr) OnComplete(summary common.JobSummary) {
...
func (lcm *lifecycleMgr) Info(msg string) {
	...
	lcm.msgQueue <- common.OutputMessage{
		MsgContent: infoMsg,
		MsgType:    common.EOutputMessageType.Info(),
	}
}
...
func (lcm *lifecycleMgr) Prompt(message string, details common.PromptDetails) common.ResponseOption {
	...
	lcm.msgQueue <- common.OutputMessage{
		MsgContent:    message,
		MsgType:       common.EOutputMessageType.Prompt(),
...
func (lcm *lifecycleMgr) Dryrun(o common.OutputBuilder) {
	dryrunMessage := ""
	if o != nil {
	...
	lcm.msgQueue <- common.OutputMessage{
		...
		MsgType:    common.EOutputMessageType.Dryrun(),
	}
}

func (lcm *lifecycleMgr) Output(o common.OutputBuilder, MsgType common.OutputMessageType) {
	om := ""
	if o != nil {
...
func (lcm *lifecycleMgr) Error(msg string) {
	...
	lcm.msgQueue <- common.OutputMessage{
		MsgContent: msg,
		MsgType:    common.EOutputMessageType.Error(),
...
func (lcm *lifecycleMgr) Exit(o common.OutputBuilder, applicationExitCode common.ExitCode) {
	if applicationExitCode != common.EExitCode.NoExit() {
		// Check if need to do memory profiling, and do memory profiling accordingly before azcopy exits.
		lcm.checkAndTriggerMemoryProfiling()
	...
	lcm.msgQueue <- common.OutputMessage{
		...
		MsgType:    common.EOutputMessageType.EndOfJob(),
...
func (lcm *lifecycleMgr) Response(resp common.LCMMsgResp) {
	...
	if lcm.outputFormat == common.EOutputFormat.Json() {
	...
	lcm.msgQueue <- common.OutputMessage{
		MsgContent: respMsg,
		MsgType:    common.EOutputMessageType.Response(),
	}
}
...
func (lcm *lifecycleMgr) processOutputMessage() {
	...
	for {
		...
		case common.EOutputFormat.Text():
			lcm.processTextOutput(msgToPrint)
		case common.EOutputFormat.None():
			lcm.processNoneOutput(msgToPrint)
		default:
...
func (lcm *lifecycleMgr) processNoneOutput(msgToOutput common.OutputMessage) {
	if msgToOutput.MsgType == common.EOutputMessageType.Error() {
		lcm.closeFunc()
		os.Exit(int(common.EExitCode.Error()))
...
func (lcm *lifecycleMgr) processJSONOutput(msgToOutput common.OutputMessage) {
	...
	fmt.Println(common.GetJsonStringFromTemplate(newJsonOutputTemplate(MsgType, msgToOutput.MsgContent,
	...
	if msgToOutput.ShouldExitProcess() {
		lcm.closeFunc()
		os.Exit(int(msgToOutput.ExitCode))
	} else if MsgType == common.EOutputMessageType.Prompt() {
...
func (lcm *lifecycleMgr) processTextOutput(msgToOutput common.OutputMessage) {
	...
	case common.EOutputMessageType.Error(), common.EOutputMessageType.EndOfJob():
	...
	case common.EOutputMessageType.Progress():
	...
	case common.EOutputMessageType.Prompt():
		questionTime := time.Now()
...
func shouldQuietMessage(msgToOutput common.OutputMessage, quietMode common.OutputVerbosity) bool {
	messageType := msgToOutput.MsgType

	switch quietMode {
	case common.EOutputVerbosity.Default():
		return false
	case common.EOutputVerbosity.Essential():
		return messageType == common.EOutputMessageType.Progress() || messageType == common.EOutputMessageType.Info() || messageType == common.EOutputMessageType.Prompt()
	case common.EOutputVerbosity.Quiet():
		return true
	default:
		return false
	}
