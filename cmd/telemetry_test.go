...
func TestTelemetryHelpDisclosesStorageAccountNames(t *testing.T) {
	flag := rootCmd.PersistentFlags().Lookup("disable-telemetry")
	if assert.NotNil(t, flag) {
		assert.Contains(t, flag.Usage, "Azure Storage account names")
		assert.Contains(t, flag.Usage, "pseudonymous")
		assert.Contains(t, flag.Usage, "AZCOPY_DISABLE_TELEMETRY")
		assert.NotContains(t, flag.Usage, "contains no PII")
	}
}
...
func TestTelemetryValuePoliciesAreReviewed(t *testing.T) {
	for flag, policy := range telemetryFlagValuePolicies {
		normalizer := reflect.ValueOf(policy.normalize).Pointer()
		if normalizer == reflect.ValueOf(normalizeTelemetryCategory).Pointer() || normalizer == reflect.ValueOf(normalizeTelemetryCategoryList).Pointer() {
			assert.Contains(t, telemetryCategoryValues, flag, "category values require a reviewed allowlist")
		}
...
func TestTelemetryCategoriesRejectUnvalidatedValues(t *testing.T) {
	for _, test := range []struct{ flag, value string }{
		{"s2s-handle-invalid-metadata", common.EInvalidMetadataHandleOption.ExcludeIfInvalid().String()},
		{"posix-properties-style", common.EPosixPropertiesStyle.AMLFS().String()},
		{"from-to", common.EFromTo.FileSMBFileNFS().String()},
		{"local-hash-storage-mode", common.EHashStorageMode.HiddenFiles().String()},
	} {
		assert.True(t, telemetryCategoryAllowed(test.flag, strings.ToLower(test.value)), test)
	}
	assert.False(t, telemetryCategoryAllowed("login-type", "device,msi"))
	for flag := range telemetryCategoryValues {
		for _, value := range []string{"private-category-canary", "https://account/path?sig=canary", "device,private-canary", "device msi"} {
			command := &cobra.Command{Use: "login"}
			command.Flags().String(flag, "", "")
			assert.NoError(t, command.Flags().Set(flag, value))
			assert.NotContains(t, telemetryOptions(command).Values, telemetryFlagValuePolicies[flag].property, flag)
		}
	}
	command := &cobra.Command{Use: "login"}
	command.Flags().String("login-type", "", "")
	assert.NoError(t, command.Flags().Set("login-type", "MSI"))
	assert.Equal(t, "msi", telemetryOptions(command).Values["OptLoginType"])
	for _, value := range []string{"NaN", "+Inf", "-Inf"} {
		_, valid := normalizeTelemetryFloat(value)
